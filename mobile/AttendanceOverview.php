            ...
            <div class="space-y-3" id="attendance-list">
                <?php 
                    ...
                    foreach ($reversed_entries as $entry): 
                        ...
                        // Determine if there is an active pending request and prepare the alert message
                        $has_pending = !empty($entry['pending_adj']) || !empty($entry['pending_leave']);
                        $alert_message = '';
                        if (!empty($entry['pending_adj'])) {
                            $alert_message = 'A time adjustment request is already pending for this date.';
                        } elseif (!empty($entry['pending_leave'])) {
                            $alert_message = 'A leave request is already pending for this date.';
                        }

                        $card_href = $has_pending ? 'javascript:void(0)' : 'time_entry?date=' . $entry['entry_date'];
                        
                        // SweetAlert Implementation Here
                        $card_onclick = $has_pending ? 'onclick="Swal.fire({icon: \'warning\', title: \'Pending Request\', text: \'' . addslashes($alert_message) . '\', confirmButtonColor: \'#1c212d\'}); return false;"' : '';
                    ?>
                <!-- Attendance Card -->
                <a href="<?= $card_href ?>" <?= $card_onclick ?> data-status="<?= $status_val ?>" class="attendance-card block bg-white rounded-xl shadow-sm border border-gray-100 p-3 flex items-start gap-4 hover:shadow-md transition duration-200">
