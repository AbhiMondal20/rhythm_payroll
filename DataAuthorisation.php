...
.di-history-link:hover { text-decoration: underline; }
...
.di-step.done .di-step-circle span::before { content: '✓'; }
.di-step.done .di-step-circle span { font-size: 0; } /* hide number */
...
.di-step.active .di-step-label { color: #2563EB; font-weight: 600; }
.di-step.done   .di-step-label { color: #374151; }
...
.di-step-line {
  ...
  margin-bottom: 26px; /* align with circles */
  min-width: 60px;
  max-width: 180px;
}
...
.di-select:focus { border-color: #2563EB; }
...
.di-file-label.chosen { color: #1e293b; }
.di-btn-choose {
  display: inline-flex;
  align-items: center;
  padding: 10px 20px;
...
.di-btn-choose:hover { background: #1D4ED8; }

.di-help-section { text-align: center; margin-bottom: 4px; }
.di-help-text { font-size: 13px; color: #64748B; margin-bottom: 6px; }
.di-help-links { display: flex; align-items: center; justify-content: center; gap: 10px; }
.di-link { font-size: 13px; color: #2563EB; text-decoration: none; cursor: pointer; }
.di-link:hover { text-decoration: underline; }
.di-link-sep { color: #CBD5E1; }
...
.di-preview-table tbody tr:hover td { background: #F8FAFC; }

/* ── Step 4 states ── */
.di-import-progress, .di-import-done, .di-import-error {
  text-align: center;
  padding: 24px 0;
}
.di-big-spinner {
...
.di-import-progress p { font-size: 14px; color: #64748B; }
.di-done-icon {
...
.di-done-msg  { font-size: 14px; color: #166534; margin-bottom: 20px; }
.di-error-icon {
  width: 52px; height: 52px;
  border-radius: 50%;
  background: #EF4444;
...
.di-error-msg { font-size: 14px; color: #991B1B; margin-bottom: 20px; }
.di-import-summary { font-size: 13.5px; color: #475569; text-align: center; margin-bottom: 6px; }
...
.di-btn-next:hover    { background: #1D4ED8; }
.di-btn-next:disabled { background: #93C5FD; cursor: not-allowed; }
.di-btn-back {
  padding: 9px 24px;
...
.di-btn-back:hover { background: #F8FAFC; border-color: #94A3B8; }
...
.di-history-title { font-size: 15px; font-weight: 700; color: #0F172A; margin: 0 0 4px; }
.di-history-note  { font-size: 12px; color: #94A3B8; margin: 0; }
.di-history-actions { display: flex; gap: 10px; }
.di-btn-outline {
  padding: 8px 20px;
  border: 1.5px solid #CBD5E1;
  background: #fff;
...
.di-btn-outline:hover { background: #F8FAFC; border-color: #94A3B8; }
...
.di-search-icon { flex-shrink: 0; margin-right: 8px; }
.di-history-search {
  flex: 1;
  border: none;
  outline: none;
...
.di-history-table thead tr { background: #F8FAFC; }
.di-history-table th {
  padding: 11px 16px;
  text-align: left;
  font-size: 11.5px;
...
.di-history-table tbody tr:last-child td { border-bottom: none; }
.di-history-table tbody tr:hover td { background: #F8FAFC; }

.di-status-completed { color: #16A34A; font-weight: 600; }
.di-status-error     { color: #DC2626; font-weight: 600; }
.di-status-progress  { color: #D97706; font-weight: 600; }

.di-file-link { color: #2563EB; text-decoration: none; font-size: 13px; }
.di-file-link:hover { text-decoration: underline; }
...
.di-info-btn:hover { color: #1D4ED8; }
...
.di-modal-title { font-size: 15px; font-weight: 700; color: #0F172A; }
.di-modal-close {
  background: none; border: none; cursor: pointer;
  font-size: 17px; color: #EF4444;
  padding: 2px 8px; border-radius: 4px;
  transition: background .1s;
}
.di-modal-close:hover { background: #FEF2F2; }
.di-modal-body {
...
.di-modal-body ul { margin: 8px 0 0 16px; padding: 0; }
.di-modal-body li { margin-bottom: 6px; }
...
.di-toast.success { background: #166534; }
.di-toast.error   { background: #991B1B; }
.di-toast.show    { opacity: 1; transform: translateY(0); }

/* ---------- Responsive ---------- */
@media (max-width: 640px) {
  .di-step-label { font-size: 11px; }
  .di-card { padding: 20px 16px; }
  .di-step1-body { flex-direction: column; }
  .di-select { max-width: 100%; }
}
</style>
...
<div class="di-wrapper">
  ...
  <div id="viewWizard">
    ...
    <div class="di-card" id="step2" style="display:none;">
      ...
      <div class="di-upload-area">
        <div class="di-file-row">
          ...
          <input type="file" id="diFileInput" accept=".csv,.xlsx,.xls"
                 style="display:none;" onchange="DI.onFileChosen(this)">
        </div>
      </div>
      <div class="di-help-section">
        ...
        <div class="di-help-links">
          ...
          <a href="#" class="di-link" onclick="DI.downloadTemplate(event)">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style="vertical-align:-2px;margin-right:3px;">
              <path d="M7 1v8M4 6l3 3 3-3" stroke="#2563EB" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  </div><div id="viewHistory" style="display:none;">
    ...
    <div class="di-history-search-wrap">
      ...
      <input type="text" class="di-history-search" id="diHistorySearch"
             placeholder="Search imports..." oninput="DI.filterHistory(this.value)">
    </div>
    ...
    <div class="di-history-table-wrap">
      <table class="di-history-table" id="diHistoryTable">
        ...
        <tbody id="diHistoryTbody">
          <tr><td colspan="5" class="di-loading-row">
...
</div><div class="di-overlay" id="diOverlay" onclick="DI.closeErrorModal()" style="display:none;"></div>
...
const DI = {
  ...
  renderPreview: function(headers, rows, totalRows) {
    ...
    if (headers && headers.length) {
      headers.forEach(h => { html += `<th>${h || 'N/A'}</th>`; });
    }
    html += '</tr></thead><tbody>';
    ...
    if (rows && rows.length) {
      ...
      html += `<tr><td colspan="${headers.length}" style="text-align:center;">No data found in file.</td></tr>`;
    }
    html += '</tbody></table>';
  ...
  refreshHistory: async function() {
    ...
    tbody.innerHTML = '<tr><td colspan="5" class="di-loading-row"><span class="di-spinner"></span> Loading…</td></tr>';
    ...
    try {
      ...
      if (data.success) {
        ...
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:#EF4444;">${data.message || 'Failed to load history.'}</td></tr>`;
      }
    } catch (err) {
      tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; color:#EF4444;">Network error fetching history.</td></tr>`;
    }
  },
  ...
  viewInstructions: async function(e) {
    ...
    document.getElementById('diInstrBody').innerHTML = '<div style="text-align:center; padding: 20px;"><span class="di-spinner"></span> Loading...</div>';
    document.getElementById('diInstrOverlay').style.display = 'block';
    document.getElementById('diInstrModal').style.display = 'block';

    try {
      const response = await fetch(`${API_URL}?action=instructions&type=${encodeURIComponent(this.selectedDataType)}`);
      const data = await response.json();
      ...
      document.getElementById('diInstrBody').innerHTML = `<p style="color:red;">Failed to load instructions.</p>`;
    }
  },
