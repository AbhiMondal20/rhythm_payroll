...
// 3. Fetch Monthly Attendance Stats (Paid, Unpaid, Short Hrs)
$paid_days = 0;
$unpaid_days = 0;
$short_hrs = "0:00";
...
// 5. Fetch Upcoming Holidays (Global, no emp_id filter needed)
$upcoming_holidays = [];
...
<head>
    ...
    <style>
        ...
        /* Custom thin scrollbar for holidays */
...
<body class="bg-gray-100 flex justify-center min-h-screen">
    ...
    <div class="w-full max-w-md bg-[#f4f5f9] min-h-screen relative flex flex-col font-sans shadow-2xl overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto no-scrollbar pb-6 flex flex-col">
                ...
                <div class="w-full relative pt-4">
                    <div id="card-slider" class="flex overflow-x-auto snap-x snap-mandatory no-scrollbar w-full pb-2 cursor-grab">
                                ...
                                <div class="max-h-[160px] overflow-y-auto thin-scrollbar pr-2">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($leaves as $index => $leave): 
                                            // Assign dynamic colors based on index for the progress bars
                                            $bg_colors = ['bg-[#fcead8]', 'bg-[#e3eef9]', 'bg-[#eef9f2]'];
                                            $fill_colors = ['bg-[#f7c28a]', 'bg-[#66aef1]', 'bg-[#6ee7b7]'];
                                            $color_idx = $index % 3;
                                            
                                            $acc = floatval($leave['accumulated']) > 0 ? floatval($leave['accumulated']) : 1; // Prevent division by zero
                        ...
                        <!-- Card 3: Upcoming Holidays (Scrollable) -->
                        <div class="min-w-full flex-shrink-0 snap-center px-4">
                ...
                <div class="bg-white rounded-t-3xl shadow-[0_-4px_10px_-2px_rgba(0,0,0,0.03)] mt-2 px-5 pt-2 pb-8 flex-1">
                    ...
                    <div class="grid grid-cols-3 gap-y-7 gap-x-2">
                        <a href="#" class="flex flex-col items-center hover:opacity-80 transition">
                            <div class="w-[50px] h-[50px] bg-[#f4f5f9] rounded-full flex items-center justify-center mb-2 text-[#5c6e8a] text-lg"><i class="fa-solid fa-clock"></i></div>
                            <span class="text-[11px] text-gray-800 font-medium text-center">Check In / Out</span>
                        </a>
                        <a href="ApplyLeave" class="flex flex-col items-center hover:opacity-80 transition">
                        ...
                        <a href="AttendanceOverview" class="flex flex-col items-center hover:opacity-80 transition">
                        ...
                        <a href="#" class="flex flex-col items-center hover:opacity-80 transition">
                            <div class="w-[50px] h-[50px] bg-[#f4f5f9] rounded-full flex items-center justify-center mb-2 text-[#5c6e8a] text-lg"><i class="fa-solid fa-shield-halved"></i></div>
                            <span class="text-[11px] text-gray-800 font-medium text-center">Policies & Links</span>
                        </a>
                    </div>
                </div>
            </main>
        ...
        <!-- ========================================== -->
        <!-- SETTINGS / PROFILE SCREEN (Hidden Initially) -->
        <!-- ========================================== -->
            ...
            <main class="flex-1 overflow-y-auto pb-8">
                ...
                <div class="px-4 space-y-3">
                    ...
                    <a href="#" class="block bg-white p-4 rounded-[10px] shadow-sm flex items-center hover:bg-gray-50 transition">
                        ...
                        <span class="text-gray-900 font-medium text-[15px]">About Perk Payroll</span>
                    </a>

                    <!-- Make sure to link to your actual logout endpoint -->
