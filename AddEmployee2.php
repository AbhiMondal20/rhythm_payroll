...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form_action'] ?? '') !== 'delete_employee') {
    ...
    $data = [
        ...
        'ctc_template_id'=> clean_post('ctc_template_id') !== '' ? (int)clean_post('ctc_template_id') : null,
        'approval_attendance' => isset($_POST['approval_attendance']) ? 1 : 0,
        'approval_leave'      => isset($_POST['approval_leave']) ? 1 : 0,
    ...
    try {
        if ($emp_id > 0) { 
            ...
            $sql = "INSERT INTO employees (`" . implode('`,`', $cols) . "`, created_at, updated_at) VALUES (" . implode(',', $vals) . ", NOW(), NOW())";
            if (!mysqli_query($conn, $sql)) throw new Exception(mysqli_error($conn));
...
if ($is_edit) {
    ...
    if (empty(trim($row['first_name'])) && empty(trim($row['last_name'])) && !empty(trim($row['employee_name']))) {
        $parts = preg_split('/\s+/', trim($row['employee_name']));
        if (count($parts) === 1) {
    ...
    $emp = array_merge($emp, [
        ...
        'approval_attendance' => $row['approval_attendance'] ?? 0,
        'approval_leave' => $row['approval_leave'] ?? 0,
...
<div class="add-emp-wrap">
    ...
    <div>
        <form id="empForm" method="POST" action="<?= htmlspecialchars($_SERVER['REQUEST_URI']) ?>" enctype="multipart/form-data" autocomplete="off" novalidate>
            <?php if ($is_edit): ?>
                <input type="hidden" name="emp_id" value="<?= (int)$emp['id'] ?>">
            <?php endif; ?>
                ...
                <div class="form-block">
                    ...
                    <div class="form-block-body">
                        <div class="fg-row col-4">
                            <div class="fg">
                                <label>TITLE</label>
                                <select name="title" id="fTitle" onchange="updatePreview(); autoSelectGender(this.value);">
                        ...
                        <div class="fg-row col-3">
                            ...
                            <div class="fg">
                                <label>GENDER</label>
                                <select name="gender" id="fGender" onchange="updatePreview()">
                                    <option value="">Select</option>
            ...
            <div class="form-section" id="section-2">
                ...
                <div class="form-block">
                    <div class="form-block-header">
                        <div class="form-block-icon" style="background:#E0E7FF; color:#4F46E5;">
                        ...
                        <div>
                            ...
                            <p>Grant permission to approve module requests</p>
                        </div>
                    </div>
                    <div class="form-block-body">
                        <div style="display:flex; gap:24px; align-items:center;">
                            <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                <input type="checkbox" name="approval_attendance" value="1" <?= $emp['approval_attendance'] ? 'checked' : '' ?> style="width:18px;height:18px;accent-color:#4F46E5;">
                                <span style="font-size:14px; font-weight:600; color:#374151;">Attendance Approvals</span>
                            </label>
                            <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                                <input type="checkbox" name="approval_leave" value="1" <?= $emp['approval_leave'] ? 'checked' : '' ?> style="width:18px;height:18px;accent-color:#4F46E5;">
                                <span style="font-size:14px; font-weight:600; color:#374151;">Leave Approvals</span>
                            </label>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="fg-row col-2">
                            <div class="fg">
                                ...
                                <select name="qualification">
                                    <option value="">Select</option>
                                    <?php foreach (['10th/SSLC','12th/HSC','Diploma','BA','MBA','Graduate','B.Sc/BCA','MBA/MCA','MBBS','MD/MS','DM/MCh','PhD'] as $q): ?>
...
function autoSelectGender(title) {
    const genderSelect = document.getElementById('fGender');
    if (!genderSelect) return;
    if (title === 'Mr.') genderSelect.value = 'Male';
    else if (title === 'Mrs.' || title === 'Ms.') genderSelect.value = 'Female';
}
...
function nextSection() { if (currentSection < totalSections) goToSection(currentSection + 1); }
function prevSection() { if (currentSection > 1) goToSection(currentSection - 1); }
...
function submitEmployeeForm() {
    ...
    if (!ok) return showEmpToast('warning', 'Please fill all required fields.');
    form.submit();
}

function confirmDelete(id) {
    if (confirm('Delete this employee?')) document.getElementById('deleteForm').submit();
}
...
function previewPhoto(event) {
    ...
    reader.onload = function(e) {
        ...
        if (img1) { img1.src = e.target.result; img1.style.display = 'block'; }
        if (img2) { img2.src = e.target.result; img2.style.display = 'block'; }
...
function updatePreview() {
    ...
    if (document.getElementById('epcDesig')) document.getElementById('epcDesig').textContent = desig;
    if (document.getElementById('epcDeptBadge')) document.getElementById('epcDeptBadge').textContent = dept;
    if (document.getElementById('epcId')) document.getElementById('epcId').textContent = empCode;
    if (document.getElementById('epcJoined')) document.getElementById('epcJoined').textContent = joined;
    if (document.getElementById('epcPhone')) document.getElementById('epcPhone').textContent = phone;
    if (document.getElementById('epcEmail')) document.getElementById('epcEmail').textContent = email;
    ...
    if (!avatarImg || avatarImg.style.display === 'none' || avatarImg.getAttribute('src') === '') {
        if (document.getElementById('epcInitials')) document.getElementById('epcInitials').textContent = initials;
        if (document.getElementById('photoInitials')) document.getElementById('photoInitials').textContent = initials;
    }
}
