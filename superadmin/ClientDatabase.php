        ...
        if ($stmt) {
            ...
            if ($stmt->execute()) {
                $swal = [
                    'icon' => 'success',
                    'title' => 'Disabled!',
                    'text' => 'Database configuration disabled successfully.',
...
if ($search !== '') {
    $where[] = "(
        c.client_name LIKE ?
        OR cd.client_code LIKE ?
        OR cd.module_key LIKE ?
        OR cd.db_name LIKE ?
        OR cd.db_user LIKE ?
        OR cd.db_host LIKE ?
    )";
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px">
    ...
    <a href="AddClientDatabase" class="cd-save-btn" style="text-decoration:none">
    ...
    <div class="cd-card-head">
        ...
        <form method="GET" style="display:flex;gap:10px;flex-wrap:wrap">
            ...
            <?php if($search !== '' || $status !== ''): ?>
                <a href="ClientDatabase" class="cd-cancel-btn">Reset</a>
            <?php endif; ?>
        </form>
    </div>
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13.5px">
            <thead>
                <tr style="background:#F9FAFB;border-bottom:1px solid #E5E7EB">
                    ...
                    <th style="padding:13px 16px;text-align:left">Database</th>
                    <th style="padding:13px 16px;text-align:left">DB User</th>
                    <th style="padding:13px 16px;text-align:left">Host</th>
                    <th style="padding:13px 16px;text-align:left">Port</th>
                    ...
                    <?php $i=1; while($row = $clients->fetch_assoc()): ?>
                        <tr style="border-bottom:1px solid #F3F4F6">
                            ...
                            <td style="padding:14px 16px">
                                <div style="display:flex;align-items:center;gap:10px">
                                    <?php if(!empty($row['logo'])): ?>
                                        <img src="../<?= e($row['logo']) ?>"
                                             style="width:42px;height:42px;border-radius:10px;object-fit:cover;border:1px solid #E5E7EB">
                                    <?php else: ?>
                                        <div style="width:42px;height:42px;border-radius:10px;background:#EFF6FF;color:#2563EB;display:flex;align-items:center;justify-content:center;font-weight:700">
                                            <?= strtoupper(substr($row['client_name'] ?: $row['client_code'], 0, 1)) ?>
                                        </div>
                                    <?php endif; ?>

                                    <div>
                                        <div style="font-weight:700;color:#111827">
                                            <?= e($row['client_name'] ?: 'N/A') ?>
                                        </div>
                            ...
                            <td style="padding:14px 16px">
                                ...
                                <div style="font-size:12px;color:#6B7280">
                                    Created: <?= e(date('d M, Y h:i A', strtotime($row['created_at']))) ?>
                                </div>
                            </td>
                            ...
                            <td style="padding:14px 16px;text-align:right">
                                <div style="display:flex;gap:8px;justify-content:flex-end">
                                    <a href="AddClientDatabase?id=<?= (int)$row['id'] ?>"
                                       class="cd-test-btn"
                                    ...
                                    <?php if($row['status']==='active'): ?>
                                        <button type="button"
                                                onclick="disableDatabase(<?= (int)$row['id'] ?>)"
                                                class="cd-cancel-btn"
...
<script>
function disableDatabase(id) {
    Swal.fire({
        icon: 'warning',
        title: 'Disable Database?',
        text: 'This database configuration will be marked inactive.',
        ...
        if (result.isConfirmed) {
            window.location.href = 'ClientDatabase?disable=' + id;
        }
    });
}
</script>
